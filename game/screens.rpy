...
label change_location:
    python:
        possible_locations = []
        for rm in cur_map.rooms.keys():
            if (rm == my_actor.room):
                continue
            player_count = 0
            for obj in network_objects:
                if (obj.type == "player"):
                    pl_actor = get_player_actor(obj.id)
                    if pl_actor.room == rm:
                        player_count += 1
            possible_locations.append((cur_map.rooms[rm].name + "(" + str(player_count) + "/" + str(cur_map.rooms[rm].cap) + ")",rm))
        possible_locations.append(("Nevermind","nevermind"))
        chosen_location = renpy.display_menu(possible_locations)
        if chosen_location != "nevermind":
            net.send(network_create_packet({
                "type":"change_room",
                "room":chosen_location
            }),False)
    return
...
label ask_for_chat:
    python:
        chat_attempt = renpy.input("Chat Message:",default="",length=64)
        if (chat_attempt != ""):
            net.send(network_create_packet({
                "type":"chat",
                "message":chat_attempt
            }),False)
    # jump network_loop
    return

label change_character:
    $ char_attempt = "monika 1a"
    menu:
        "Monika":
            $ char_attempt = "monika "
            jump chn_monika_body
        "Sayori":
            $ char_attempt = "sayori "
            jump chn_sayori_body
        "Natsuki":
            $ char_attempt = "natsuki "
            jump chn_natsuki_body
        "Yuri":
            $ char_attempt = "yuri "
            jump chn_yuri_body
        "MC":
...
label chn_monika_body:
    menu:
        ...
        "Leaning Forward":
            $ char_attempt += "5"
            jump chn_monika_special
    jump chn_monika_face
...
label chn_mc_body:
    menu:
        ...
        "Scratching Head":
            $ char_attempt += "2"
        "Scratching Head w/ Hand In Pocket":
            $ char_attempt += "4"
        "Hand In Pocket":
            $ char_attempt += "3"
        "Arms Crossed":
...
label chn_natsuki_body:
    menu:
        ...
        "Scream":
            $ char_attempt += "scream"
            call chn_casual_fit from _call_chn_casual_fit
            jump chn_send_data
    call chn_casual_fit from _call_chn_casual_fit_1
    jump chn_natsuki_face
...
label chn_yuri_body:
    menu:
        ...
        "Looking Away":
            $ char_attempt += "4"
            jump chn_yuri_special
    call chn_casual_fit from _call_chn_casual_fit_2
    jump chn_yuri_face

label chn_sayori_body:
    menu:
        "Standard":
            $ char_attempt += "1"
        "Right Arm Raised":
        ...
        "Tapping Fingers":
            $ char_attempt += "5"
            jump chn_sayori_special
    call chn_casual_fit from _call_chn_casual_fit_3
    jump chn_sayori_face

label chn_monika_face:
    menu:
        "Smile":
            $ char_attempt += "b"
        "Neutral":
...
label chn_yuri_face:
    menu:
        ...
        "Concerned":
            $ char_attempt += "t"
        "Yandere Expressions":
            jump chn_yuri_yandere_face
    jump chn_send_data

label chn_yuri_yandere_face:
    menu:
        "Yandere Happy":
            $ char_attempt += "y1"
        "Yandere Pissed":
...
label chn_sayori_special:
    menu:
        "Smiling Towards Viewer":
            $ char_attempt += "a"
        "Smiling Away From Viewer":
            $ char_attempt += "b"
        "Pouting Towards Viewer":
            $ char_attempt += "c"
        "Pouting Away From Viewer":
            $ char_attempt += "d"
    jump chn_send_data

label chn_yuri_special:
    menu:
...
label chn_send_data:
    python:
        net.send(network_create_packet({
            "type":"change_char",
            "character":char_attempt
        }),False)
    return

label change_music:
    menu:
        "Other players wont be able to hear your music."
...
label chm_creepy:
    menu:
        ...
        "Sayonara":
            play music td
            jump chm_end
        "Play With Me(Distorted)":
            play music t6g
            jump chm_end
        "Poem Panic(Distorted)":
            play music t7g
            jump chm_end
        "Ghost Menu":
            play music ghostmenu
            jump chm_end
        "Main Theme(Distorted)":
            play music t3g3
            jump chm_end
        "Heartbeat":
            play music hb
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            #textbutton _("Disconnect") action If(in_net_loop == True,true=Jump("disconnect"))
            textbutton _("History") action ShowMenu('history')
            textbutton _("Chat") action If(in_net_loop == True,true=Call("ask_for_chat"))
            textbutton _("Character") action If(in_net_loop == True,true=Call("change_character"))
            textbutton _("Location") action If(in_net_loop == True,true=Call("change_location"))
            textbutton _("Music") action If(in_net_loop == True,true=Call("change_music"))
    ...
    vbox:
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                textbutton _("Connect") action Show(screen="name_input", message="Please enter a username.", ok_action=Function(FinishEnterName))
            ...
            elif not main_menu and not in_net_loop:
            ...
            if renpy.variant("pc"):
                ...
                #else:
                    #textbutton _("Disconnect") action Jump("disconnect")
        else:
    ...
    use game_menu(_("Credits"), scroll="viewport"):
        ...
        window:
            ...
            vbox:
                ...
                text "MC Sprites by Childish-N (https://www.deviantart.com/childish-n)"
